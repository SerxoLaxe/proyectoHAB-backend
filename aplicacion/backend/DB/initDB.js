...
/**
 * Crea las tablas del array de tablas del módulo tablasDD si es que no existen
 * en la base de datos.
 *
 * @param {Object} conexion - Conexión a Mysql
 */
...
async function llenarTablaUsuarios(numeroDeUsuarios, conexion) {
  for (let i = 0; i < numeroDeUsuarios; i++) {
    ...
    const biografia = faker.lorem.paragraph(
      fakerConfig.usuarios.parrafosBiografia
    );
...
async function llenarTablaExperiencias(numeroDeExperiencias, conexion) {
  ...
  for (let i = 0; i < numeroDeExperiencias; i++) {
    ...
    const precio = lodash.random(
      experiencias.precio.minimo,
      experiencias.precio.maximo
    );
    const ubicacion = faker.address.city();
    const rating = lodash.random(
      experiencias.rating.minimo,
      experiencias.rating.maximo
    );
    const plazasTotales = lodash.random(
